...
図形(sp)=ノード、コネクタ(cxnSp)や線/矢印形状=エッジ候補として取り出し、
トポロジ(どのノード→どのノードか)を 2 段で復元する:
  - tier1 明示接続: コネクタの stCxn/endCxn が指す図形 id をそのまま採用(高信頼)
  - tier2 幾何推定: 明示接続が無い手置き矢印は、始点/終点アンカー座標に
                    最も近い図形へ対応づける(近似)
どちらにも解決できない矢印は skipped_connectors として数える。
"""
